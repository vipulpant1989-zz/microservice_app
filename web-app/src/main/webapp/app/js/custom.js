...
jQuery(document).ready(function($) {
	...
	(function() {
		...
		window.revolotionInit = function(){
	...
	(function(){
		//
		window.initSoundCloud = function(soundcloudUsername){
			...
			var widgetUrl = "https://soundcloud.com/"+soundcloudUsername;
	...
	(function() {
		...
		//Init Player
		window.init_player = function(ref){
		 	// Playlist functions
			$('.playlists li:odd').addClass('odd');
			...
			$('.track').each(function(){ $(this).addClass('playable music'); });
			...
			if ($('.track').data('show-ui'))
				$('.track').addClass('show-ui');
			...
			$('.music').MPlayer({
				...
				playlistSelector : '.playlists, #tracklist',
				onReady : function() {
					// Callback function
					window.music_helpers();
					window.full_player('#tracklist');	
				}
			});
		}
	
		// Helper music function
		window.music_helpers = function() {
		...
		// Full Player
		window.full_player = function(ref) {
			...
			var 
				...
				$track_number = 0;
				console.log("Listing --------- ",$list.length);
			// Update list
			...
			$('.play', $player).on('click', function(e){
				...
				if ($this_track.hasClass('playing')) {
					...
					console.log('this_track', $this_track);
	...
	(function() {
		...
		//Add Fancybox for button Click.
		window.bookingForm = function(){
			$.fancybox($('.bookForm').html(),{
				overlayOpacity : .8,
				overlayColor: '#000',
				centerOnScroll : true,
				autoScale : true
	...
	(function(){
		//id will be the band id.
		window.getTracklist = function(id){
			$.ajax({
				url : '/json/audios.json',
				dataType:'json',
				success: function(data){
					setTrackList(data,id);
				},error: function(data){
					alert('Sorry Server busy, please try after some time.');
				}
			});
		}
		window.setTrackList = function(data,id){
			$.each(data,function(key , value){
				if(Number(value.bandId) === Number(id)){
					$.each(value.audio , function(key , audio){
						$('#mytracklist').append('<li><div class="track"'
							+' data-url="'+audio.url+'">'+audio.name+'</div></li>');
					});
				}
			});
			init_player();
	...
	(function() {
		...
		window.initToolTip = function(){
			...
			$('.tip').on('mouseenter', function(e) {
				...
				if($(e.target).parent().hasClass('tip-top') === false)
					$('#tip').remove(); 
	...
	(function() {
		...
		window.initMobileNav = function(){
			...
			$nav.find('li').each(function() {
				...
				console.log('Value -----------  ',$a.attr('ui-sref'));
	...
	(function() {
		...
		if (true) {
			...
			// Add filter event
			window._items_filter = function($el, $data) {
